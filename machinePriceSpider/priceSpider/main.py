from pojo import Excel, Log, UserThread, PaijiContrast, Access
...
def import_excel():
	...
	log = Log()
	access = Access()
	paijiContrast = PaijiContrast()
	excel = Excel(log, access, paijiContrast)
...
def traverse_excel(xlrd_worksheet, xlwt_worksheet):
	...
	for i in range(userNum):
		thread = UserThread(i, threadLock, xlrd_worksheet, xlwt_worksheet, access, excel)
		thread.start()
