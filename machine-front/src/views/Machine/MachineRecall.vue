<template>
  <div>
    <el-button @click="dialogVisible = true">召回机器</el-button>
    ...
    <el-table
      ...
      <el-table-column
          prop="preOperateEmpId"
          label="召回前的操作人"
          width="170">
        <template #default="scope">
          {{ $store.state.employeeNameCorr[scope.row.preOperateEmpId] }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    initMachines() {
      getMachineRecall(this.current, this.size, {"nowOperateEmpId": this.$store.state.userId}).then((resp) => {
        if (resp.data.obj) {
      ...
      if (this.numberInput !== "") {
        ...
        getMachine(1, 10, search).then(resp => {
          if (resp.data.obj) {
            ...
            console.log(this.$store.state.empIdToStorageLocationIdsForGateCorr[this.$store.state.userId])
            console.log("aaaa")
            console.log(machine.storageLocationId)
