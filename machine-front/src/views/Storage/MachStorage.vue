    ...
    <div style="border: 1px solid #409eff; border-radius: 5px; box-sizing: border-box; padding: 5px; margin: 10px 0px">
      <el-row>
        <el-col :span="8" style="margin-right: 10px;">
          采购单号：
          <el-input v-model="searchMachine.purchaseOrder"
                    size="mini"
          ...
          <template #default="scope">
            <div>
              <el-input maxlength="9" v-show="isEdit"
                        ...
                        @change="initUpdateMachine(scope.row, 'salePrice')"></el-input>
              <span v-show="!isEdit">{{
                  scope.row.salePrice
                }}</span>
            </div>
          ...
          <template #default="scope">
            <div>
              <el-select size="mini" v-show="isEdit"
                         ...
                         @change="initUpdateMachine(scope.row, 'statusId')">
                <el-option
          ...
          <template #default="scope">
            <div>
              <el-select size="mini" v-show="isEdit"
                         ...
                         @change="initUpdateMachine(scope.row, 'saleChannelId')">
                <el-option
                    v-for="index in Object.keys($store.state.machineChannelCorr).map(Number)"
                    :key="index"
                    :label="$store.state.machineChannelCorr[index]"
        ...
        <el-table-column
          ...
          <template #default="scope">
            <div>
              ...
              <span v-show="!isEdit">{{ scope.row.enterStorageDate }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column
          ...
          <template #default="scope">
            <div>
              ...
              <span v-show="!isEdit">{{ scope.row.outStorageDate }}</span>
            </div>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <el-popover
              ...
              <el-input
                  ...
                  :disabled="!isEdit || isModifyField.comment === 0"
...
export default {
  ...
  activated() {
    if(this.$route.query.purchaseOrder !== undefined){
      this.searchMachine.purchaseOrder = this.$route.query.purchaseOrder
    }
    this.initMachine();
    this.StopTimer = true;
    let that = this;
  ...
  methods: {
    initMachine() {
      ...
      // })
      machineApi.getMachine(this.currentPage, this.size, this.searchMachine, this.bidDateScope).then(resp => {
        let obj = resp.data.obj;
        if (obj) {
          this.machines = obj.data;
          this.total = obj.total;
