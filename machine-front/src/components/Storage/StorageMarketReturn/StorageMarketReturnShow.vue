<template>
  <div>
    ...
    <MachineEdit table-name="marketReturnEnterStorage" :machine="editReceipt" :edit-show="editShow"
...
<script>
import * as marketReturnEnterStorageApi from "../../../api/marketReturnEnterStorageApi"
...
import {deleteEnterStorageReceipt} from "../../../api/enterStorageApi";

export default {
  name: "StorageMarketReturnShow",
  data() {
    return {
  ...
  methods: {
    ...
    addReceiptInfo() {
      ...
      this.$confirm('是否确定要添加', '提示', {
        ...
        createMarketReturnEnterStorageReceipt(this.newReceiptInfo).then(resp => {
    ...
    release(row) {
      ...
      this.$confirm('是否确定要发布该转交单', '提示', {
        ...
        releaseMarketReturnEnterStorageReceipt(row.id).then(resp => {
          if (resp.data.code === 200) {
    ...
    handleDelete(row) {
      this.$confirm('是否确定要删除', '提示', {
        ...
        deleteMarketReturnEnterStorageReceipt(row.id).then(resp => {
          if (resp.data.code === 200) {
