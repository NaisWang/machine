		...
		try {
			...
			if (sendFixMachineService.save(sendFixMachine)) {
				if (machineService.update(machine, new UpdateWrapper<Machine>().eq("id", machine.getId()))) {
					if (machineTraceService.save(new MachineTrace(machine.getId(),machine.getNumber(), machine.getStatusId(), receiptId, now, empId, machine.getComment(), machine.getStorageLocationId(), machine.getIsUpShelf()))) {
		...
		try {
			...
			if (machineService.update(machine, new UpdateWrapper<Machine>().eq("id", machine.getId()))) {
				...
				if (sendFixMachineService.update(new SendFixMachine(), new UpdateWrapper<SendFixMachine>().eq("send_fix_machine_id", sendFixMachine.getSendFixMachineId()).set("fix_status", 2).set("receive_time", LocalDateTime.now()))) {
					if (machineTraceService.save(new MachineTrace(machine.getId(),machine.getNumber(), machine.getStatusId(), sendFixMachine.getReceiptId(), now, empId, machine.getComment(), machine.getStorageLocationId(), machine.getIsUpShelf()))) {
